...
class oldVideosViewController: UIViewController, YTPlayerViewDelegate {
...
//    }
    ...
    func loadViewTimer(){
        loadTimer = NSTimer.scheduledTimerWithTimeInterval(1, target: self, selector: #selector(oldVideosViewController.parseJSON), userInfo: nil, repeats: false)
    ...
    func parseJSON(){
        ...
        if let videoJSONData = NSData(contentsOfURL: videoURLData) {
            ...
            let noBranchDataLabel = UILabel(frame: CGRectMake(15, 50, self.scrollView.bounds.size.width - 30, 0))
            noBranchDataLabel.text = "We apologize, but an error has occurred while getting the video data."
            ...
            self.scrollView.addSubview(noBranchDataLabel)
        }
